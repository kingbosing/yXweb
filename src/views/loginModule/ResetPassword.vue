    ...
    <el-main>
        <el-header height="720px">
           <el-card id="loginIndex" class="box-card">
               ...
               <button :class="{largeBtn:true, submitBtn:this.password!=''&&this.passwordR!=''}" @click="submitFun">下一步</button>
               <p>
...
export default {
  ...
  methods: {
    // 提交
    submitFun() {
      if ((/^(?!([a-zA-Z]+|\d+)$)[a-zA-Z\d]{6,16}$/).test(this.password) && this.passwordR == this.password) {
        const RegisterArr = JSON.parse(sessionStorage.getItem('RegisterModel'))
        const model = {
          'QueueId': this.$route.query.QueueId,
          'ValidateCode': this.$route.query.Code,
          'Password': this.password
        }
        console.log(model)
        const that = this
        login.reset_password(model).then(res => {
          console.log(res.data)
          if (res.data.successful) {
            that.$router.push({ path: '/loginModule/ResetSuccessful' })
          } else {
