<template>
     <el-container class="temlateMain apply" id="report"  v-loading.fullscreen.lock="fullscreenLoading"  element-loading-spinner="loadIcon" v-cloak>
        <el-header height="50px">
            <span :class="{tab_btn:true,active:applyAct==1}" v-on:click="applyAct=1">待我审批</span>
            <span :class="{tab_btn:true,active:applyAct==2}" v-on:click="applyAct=2">我已审批</span>
                    ...
                    <el-scrollbar class="onlyY">
                            <div v-show="totalCount>0&&listType==0" class="apl_content_list">
                                ...
                                <el-row class="col" v-for="(k,v) in listData" :key="v">
                                    <div v-on:click="toContent(k.ApplyID,k.ApprovalID)">
                                        <el-col :span="4"><div class="grid-content bg-purple"><b class='type_icon' v-bind:class="setClass(k.ApplyTypeName)"></b>{{k.ApplyTypeName}}</div></el-col>

                                        <el-col :span="4"><div class="grid-content bg-purple">{{k.SerialNumber}}</div></el-col>
                                        <el-col :span="4"><div class="grid-content bg-purple">{{k.CreateTime}}</div></el-col>
                                        <el-col :span="4" v-if="applyAct==1"><div class="grid-content bg-purple color-yello">{{k.Status==1?'审批中':k.StatusDesc}}</div></el-col>
                                        <el-col :span="4" v-else><div class="grid-content bg-purple "  v-bind:class="k.Status|setStatusColor">{{k.StatusDesc}}</div></el-col>
                                        <el-col :span="8"><div class="grid-content bg-purple">{{k.Context|toTrunString}}</div></el-col>
                                    </div>
                                </el-row>
                            </div>

                            <div v-show="totalCount>0&&listType==1" class="apl_content_box" >
                               <div class="apl_col box-card" v-for="(k,v) in listData"  v-on:click="toContent(k.ApplyID,k.ApprovalID)"  :key="v">
                                    <div class="title" v-if="applyAct===1">
                                        <b class="icon" v-bind:class="k.ApplyTypeName|setClass"></b>{{k.ApplyTypeName}}<span class="type color-yello">{{k.Status==1?'审批中':k.StatusDesc}}</span>
                                    </div>
                                    <div class="title" v-else>
                                        <b class="icon" v-bind:class="k.ApplyTypeName|setClass"></b>{{k.ApplyTypeName}}<span class="type color-yello">{{k.Status==1?'进行中':k.StatusDesc}}</span>

                                    </div>
                                   <div class="main">
                                        <p>编号：{{k.SerialNumber}}</p>
                                       <div v-html="showList(k.Context)"></div>

                                        <p>申请时间：{{k.CreateTime}}</p>
                                   </div>
...
import { ApprovalV1, Approval_historyV1, set_myListType } from '@/api/applyapproval'
...
export default {
  ...
  methods: {
    ...
    replace() {
      ...
      set_myListType(getToken(), newStats, 2).then((res) => {
        const data = res.data
        // console.log(data)
        if (data.Successful) {
    ...
    toContent: function(n, m) {
      this.$router.push({ path: '/ApplyApproval/ApprovalContent', query: { ApplyID: n, ApprovalID: m }})
    },
    setClass(data) {
      if (data == '合同') {
    ...
    toDownload() {
      window.open(globalvariable().apiurl + 'api/Apply/export_apply?Token=' + getToken() + ' &type=' + (this.applyAct == 1 ? 3 : 4) + '&searchText=' + this.input)
    }
  }
}
...
var getRecordV1 = (app, index, size, search) => {
  ...
  if (app.applyAct === 1) {
    ApprovalV1(getToken(), app.index, app.size, search).then((res) => {
    ...
    Approval_historyV1(getToken(), app.index, app.size, search).then((res) => {
      const data = res.data.Data
      //   console.log(data)
