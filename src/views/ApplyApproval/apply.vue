<template>
     <el-container class="temlateMain apply" id="report"  v-loading.fullscreen.lock="fullscreenLoading"  element-loading-spinner="loadIcon" v-cloak>
        <el-header height="50px">
            <span :class="{tab_btn:true,active:applyAct==1}" v-on:click="applyAct=1">进行中</span>
            <span :class="{tab_btn:true,active:applyAct==2}" v-on:click="applyAct=2">已完成</span>

             <div class="header-right">
                <b class="replace right" v-on:click="replace()"></b>
                <!-- <b class="refresh right" v-on:click="refresh()"></b> -->
               ...
               <el-button type="warning" size="small" @click="toApply">新建申请</el-button>
            </div>
        </el-header>
                            ...
                            <div v-show="totalCount>0&&listType==0" class="apl_content_list">
                                ...
                                <el-row class="col" v-for="(k,v) in listData" :key="v">
                                    <div v-on:click="toContent(k.ApplyID)">
                                        ...
                                        <el-col :span="4" v-if="applyAct==1"><div class="grid-content bg-purple color-yello">{{k.Status==1?'进行中':'进行中'}}</div></el-col>
                            ...
                            <div v-show="totalCount>0&&listType==1" class="apl_content_box" >
                               <div class="apl_col box-card" v-for="(k,v) in listData"  v-on:click="toContent(k.ApplyID)"  :key="v">
                                    <div class="title" v-if="applyAct===1">
                                        <b class="icon" v-bind:class="k.ApplyTypeName|setClass"></b>{{k.ApplyTypeName}}<span class="type color-yello">{{k.Status==1?'进行中':'进行中'}}</span>
                                    </div>
                                    <div class="title" v-else>
                                    <b class="icon" v-bind:class="k.ApplyTypeName|setClass"></b>{{k.ApplyTypeName}}<span class="type"  v-bind:class="k.Status|setStatusColor">{{k.Status|setStatusText}}</span>
                                    </div>
                                   <div class="main">
...
import { RecordV1, historyV1, set_myListType } from '@/api/applyapproval'
...
export default {
  ...
  methods: {
    ...
    toApply() {
      this.$router.push({ path: '/ApplyApproval/ApplyCreat' })
    },
    ...
    replace() {
      ...
      set_myListType(getToken(), newStats, 1).then((res) => {
        const data = res.data
        // console.log(data)
        if (data.Successful) {
    ...
    //   console.log(data)
      this.$router.push({ path: '/ApplyApproval/ApplyContent', query: { id: data }})
    //   location.href = '/ApplyApproval/ApplyContent?id=' + data
    },
    setClass(data) {
      if (data == '合同') {
...
var getRecordV1 = (app, index, size, search) => {
  ...
  if (app.applyAct === 1) {
    RecordV1(getToken(), app.index, app.size, search).then((res) => {
    ...
    historyV1(getToken(), app.index, app.size, search).then((res) => {
      const data = res.data.Data
      app.fullscreenLoading = false
      app.listData = data.Records
      app.totalCount = data.TotalCount
