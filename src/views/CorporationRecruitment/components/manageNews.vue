   ...
   <el-container  class="temlateMain" v-cloak  v-loading.fullscreen.lock="fullscreenLoading"  element-loading-spinner="loadIcon">
        ...
        <el-main>
            <div class="kong" v-if="totalCount==0">
                ...
                <p>还未收到投递的简历，请先发布职位！</p>
            </div>
            <div v-else class="cell" v-for="key in list" :key="key.OrderId" @click="goContent(key.Status,key.ResumeId,key.OrderId,key.ScanStatus)">
                <img src="/images/personalCard-touxiang.png" alt="" v-if="!key.Faceurl" />
                <img v-bind:src="getImgUrl(key.Faceurl,65)" alt="" v-if="key.Faceurl" />
                <div class="detail">
                    <div class="top">
                        <span class="name">{{key.Name}}</span>
                        <span>{{key.Sex?'女':'男'}}/{{key.Age}}/{{key.Education}}</span>
                        <a v-if="key.ScanStatus">已浏览</a>
                        <span class="job right">投递岗位：{{key.JobName}}</span>
                    </div>
                    <p class="bottom">
                        <span>{{key.RecentCompanyName}} {{key.RecentPositionName}} {{getTime(key.RecentWorkStartDate)}}-{{getTime(key.RecentWorkEndDate)}} {{(key.RecentWorkDetail)}}</span>
                        <b class="right time">更新时间：{{key.CreatedTime.split(' ')[0]}}</b>
                    </p>
                </div>
            </div>
