                        ...
                        <el-form-item label="负 责 人：" >
                            ...
                            <ul class="copyList">
                                <li v-for="k in MangeList" :key="k.ItemId">
                                    <a @click="removeIt(k.ItemId||k.ProfileId,1)"></a>
                                    <img class="img-circle" :src="getUrl(k.HeadUrl||k.FaceUrl)" alt="">
                                    <span>{{k.Name||k.FullName}}</span>
                                </li>
                            </ul>
                        </el-form-item>
                            ...
                            <span class="ParticipateProfile getShowDep" @click="ParticipateShow=true">选择参与人</span>
                             <ul class="copyList">
                                <li v-for="k in ParticipateList" :key="k.ItemId">
                                    <a @click="removeIt(k.ItemId||k.ProfileId,2)"></a>
                                    <img class="img-circle" :src="getUrl(k.HeadUrl||k.FaceUrl)" alt="">
                                    <span>{{k.Name||k.FullName}}</span>
                                </li>
                            </ul>
...
<script>
import { GetManageList, OperateTask, GetTaskForModify } from '@/api/task.js'
...
export default {
  ...
  mounted() {
    GetDetail(this.$route.query.id, this)
  },
  watch: {
    MangeList() {
      this.MangeAct_List = []
      for (let i = 0; i < this.MangeList.length; i++) {
        this.MangeAct_List.push(this.MangeList[i].ItemId || this.MangeList[i].ProfileId)
      }
    },
    ParticipateList() {
      this.ParticipateAct_List = []
      for (let i = 0; i < this.ParticipateList.length; i++) {
        this.ParticipateAct_List.push(this.ParticipateList[i].ItemId || this.ParticipateList[i].ProfileId)
      }
    }
    ...
    removeIt(id, state) {
      if (state == 1) {
        this.MangeList = this.MangeList.filter(item => {
          return (item.ItemId || item.ProfileId) != id
        })
      } else if (state == 2) {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          OperateTask(getToken(), vData, 'update', this.$route.query.id).then((res) => {
            // console.log(res)
...
function GetDetail(id, app) {
  // console.log(id);
  GetTaskForModify(getToken(), id).then((res) => {
    var data = res.data
    // console.log(data);
    app.fullscreenLoading = false
    if (data.Successful) {
      var GetArr = data.Data
      app.ruleForm.name = GetArr.Name
      app.ruleForm.desc = GetArr.TaskDescription
      app.ruleForm.region = GetArr.ImportantDegree + ''
      app.ruleForm.date1 = [GetArr.BeginTime, GetArr.EndTime]
      app.ruleForm.resource = GetArr.TaskType + ''
      app.ruleForm.delivery = GetArr.TaskOnOff
      app.ruleForm.speedPercent = GetArr.SpeedPercent
      app.MangeList = GetArr.ManageProfileList
      app.ParticipateList = GetArr.ParterProfileList
    }
  })
}
</script>
  ...
  .el-main{
    ...
    .main_task{
        ...
        .title{
            ...
            // float: left;
            // margin-left: 435px;
            background: url(/images/Task/edit_task.png) no-repeat;
        }
        .el-select{
            width: 100%;
        }
