<template>
    <el-container v-loading.fullscreen.lock="fullscreenLoading"  element-loading-spinner="loadIcon" v-cloak>
       <el-header>
            <el-row>
                <el-col :span="3"><div class="grid-content bg-purple">工作项类别<b>*</b></div></el-col>
                <el-col :span="3"><div class="grid-content bg-purple">类别描述</div></el-col>
       ...
       <!--**********弹窗************-->
        <el-dialog :title="titleStr" :visible.sync="dialogFormVisible" class="hardAndEasy" width="760px">
            <el-form :model="form" :rules="rules" ref="ruleForm" label-width="100px" label-position="demo-ruleForm">
                <el-form-item label="工作项类别:" prop="name">
                    <el-input v-model="form.name" :maxlength="15" v-on:keyup.native="DelK()" placeholder="请输入工作项名称"></el-input>
                </el-form-item>
                <el-form-item label="类 别 描 述:" prop="describe">
                    <el-input v-model="form.describe" :maxlength="30" v-on:keyup.native="DelE()" placeholder="请输入描述"></el-input>
                </el-form-item>
...
export default {
  ...
  methods: {
    ...
    submitForm(formName) { // 提交
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (Data.Id == '') { // 新建
            const that = this
            performanceAppraisal.WorkItemType_create(getToken(), Data).then(res => {
            ...
            const that = this
            performanceAppraisal.WorkItemType_edit(getToken(), Data).then(res => {
              console.log(res.data)
              if (res.data.Successful) {
                that.$message({
                  message: '编辑成功',
    ...
    toOpen(ID) {
      console.log(ID)
      if (ID == '') {
        this.titleStr = '新建工作类别'
        this.dialogFormVisible = true
        this.funId = ''
        this.resetForm('ruleForm')
      } else {
        this.titleStr = '编辑工作类别'
        this.dialogFormVisible = true
        this.funId = ID
        const that = this
        performanceAppraisal.WorkItemType_get_info(getToken(), ID).then(res => {
          console.log(res.data)
          if (res.data.Successful) {
    ...
    Enable(id, num) {
      console.log(id, num)
      const that = this
      performanceAppraisal.WorkItemType_set_status(getToken(), id, num).then(res => {
        console.log(res.data)
        if (res.data.Successful) {
          that.$message({
            message: '操作成功',
    ...
    getList(pageIndex, pageSize) {
      ...
      performanceAppraisal.WorkItemType_get_list(getToken(), pageIndex, pageSize).then(res => {
        console.log(res.data)
        if (res.data.Successful) {
