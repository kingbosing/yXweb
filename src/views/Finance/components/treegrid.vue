<template>
  <div class="treegrid">
    ...
    <el-table :data="data" border :max-height="defaultmaxheight" style="width: 100%" :row-style="showTr"  :show-summary="showSummary" :summary-method="getSummaries"><!--:show-summary="showSummary"-->
      <el-table-column v-for="(column, index) in columns" :key="column.dataIndex" :label="column.text" header-align="center">
        <el-table-column v-if="column.hasOwnProperty('child')" v-for="(c_column,v) in column.child" :key="v"
            :label="c_column.text">
          ...
          <template slot-scope="scope">
            ...
            <span v-else-if="index===0" class="ms-tree-space"></span>
              <span :style="scope.row._parent?'color:#8a8a8a':'color:#333'" v-if="IsEditor(column.isEdit)"><el-input v-model="scope.row[column.dataIndex]" @blur="SaveEarly(scope.row.ID,scope.row[column.dataIndex],column.typevalue)"></el-input></span>
                <span v-else :style="scope.row._parent?'color:#8a8a8a':'color:#333'">{{scope.row[column.dataIndex]}}</span>
          </template>
      </el-table-column>
...
export default {
  ...
  computed: {
    ...
    data: function() {
      const me = this
      if (me.treeStructure) {
        const data = DataTransfer.treeToArray(
          me.dataSource,
          null,
          null,
          me.defaultExpandAll
