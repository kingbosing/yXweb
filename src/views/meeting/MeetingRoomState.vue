    ...
    <el-container id="NewMeeting" class="temlateMain">
          <el-header height="50px">
            <!--<el-breadcrumb separator-class="el-icon-arrow-right">
                <el-breadcrumb-item v-on:click="ifNext=false">会议室使用情况</el-breadcrumb-item>
                <el-breadcrumb-item v-show="ifNext">新建会议</el-breadcrumb-item>
            </el-breadcrumb>-->
            <span v-on:click="ifNext=false">会议室使用情况</span><span v-show="ifNext"><b> > </b>新建会议</span>
        </el-header>
        <el-main>
            <!--会议情况列表-->
                        ...
                        <div class="getTime_content">
                            <div id="hasMeetingRoom" v-show="!checked">
                                ...
                                <p style="text-align:center;margin:15px 0">
                                    <el-button v-show="timeList.getNum.length>0" type="warning" size="medium" v-on:click="nextStep">预 定</el-button>
                                </p>
                            </div>
                            <!--无需会议室-->
                            <!--<div v-show="checked" id="noMeetingRoom">
                                <el-form :model="ruleForm" :rules="rules2" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                                    ...
                                    <el-form-item label="会议日期" required>
                                        <el-col :span="11">
                                            <el-form-item prop="date1">
                                                <el-date-picker type="date" placeholder="选择日期" v-model="ruleForm.date1" style="width: 400px;"></el-date-picker>
                                            </el-form-item>
                                        </el-col>
                                    </el-form-item>
                                    ...
                                    <el-form-item id="">
                                        ...
                                        <el-button type="warning" size="medium" v-on:click="submitForm('ruleForm')">提交</el-button>
...
export default {
  ...
  methods: {
    ...
    getLogoFormatUrl(src) {
      if (src) {
        return urlUtil.formatPicUrl(src, 42, 42)
      } else {
        return ''
      }
    },
    ...
    saveDraft() {
      // console.log()
      var Data = getMeetingRoomInfo()
      if (
        Data.MeetingContent &&
        Data.Title &&
        Data.MeetingTypeId != '0' &&
        Data.ProfileIds
      ) {
        this.saveDraftOK = true
        /* $.ajax({
                    url: "/MyMeeting/CreateDraftMeeting",
                    type: "POST",
                    data: getMeetingRoomInfo(),
                    success: function (data) {
                        ...
                        app.$message({
                            ...
                            type: 'success',
                            onClose: function () {
                                app.saveDraftOK = false;

                                window.location.href = "/MyMeeting/Meeting_DraftLibrary";
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          Data.StartTime = data + ' ' + this.ruleForm.startTime
          Data.EndTime = data + ' ' + this.ruleForm.endTime
          Data.MeetingAddress = this.ruleForm.name

          console.log(Data)
          this.noRoomOK = true
          /* $.ajax({
                        url: "/MyMeeting/CreateMeeting",
                        type: "POST",
                        data: Data,
                        success: function (data) {
                            ...
                            app.$message({
                                ...
                                onClose: function () {
                                    ...
                                    window.location.href = "/MyMeeting/index";
                                    window.location.target = "mainframe";
                                }
                            });
                        }
    ...
    ToSubmit(infoForm) {
      this.$refs[infoForm].validate(valid => {
        if (valid) {
          ...
          Data.StartTime =
            data + ' ' + getTiime(this.timeList.getNum[0]) + ':00'

          Data.EndTime =
            data +
            ' ' +
            getTiime(
              (this.timeList.getNum[1] || this.timeList.getNum[0]) + 0.5
            ) +
            ':00'
    ...
    setClass(item, n, id, m) {
      ...
      if (this.timeList.ID == id) {
        if (this.timeList.getNum.length == 2) {
          if (
            this.timeList.getNum[0] <= item &&
            item <= this.timeList.getNum[1]
          ) {
            return 'getIt'
          } else if (
            this.timeList.getNum[1] <= item &&
            item <= this.timeList.getNum[0]
          ) {
            return 'getIt'
          }
        } else {
...
function getMeetingRoomList(theDate, mixsize, maxsize, that) {
  ...
  return new Promise((resolve, reject) => {
    meeting
      .used_times_list(getToken(), theDate, mixsize, maxsize)
      .then(response => {
        console.log('///')
        console.log(response.data)
        if (response.data.Successful) {
...
function getMeetingRoomInfo() {
  ...
  if ($('.cope-popup ul li').length == 0) {
    ...
    Data.ProfileIds = []
    for (let j = 0; j < $('.cope-popup ul li').length; j++) {
...
#NewMeeting {
  ...
  .ifHasRoom b.checked {
    background: url(/image/room_tick.png) no-repeat center;
  }

  .ifHasRoom b.noCheck {
    background: url(/image/room_choose.png) no-repeat center;
  }
  ...
  #noMeetingRoom .el-date-editor.el-input,
  ...
  #hasMeetingRoom .thead .el-table td,
  .el-table th.is-leaf {
  ...
  #hasMeetingRoom tbody td,
  #hasMeetingRoom tbody .cell {
    padding: 0;
  }
  #hasMeetingRoom .cell {
    text-align: center;
